...
zx_status_t PlatformI2cBus::Start() {
    auto status = i2c_impl_get_max_transfer_size(&i2c_, bus_id_, &max_transfer_);
    if (status != ZX_OK) {
        return status;
    }
    ...
    thrd_create_with_name(&thread_, I2cThread, this, name);
    return ZX_OK;
}
...
void PlatformI2cBus::I2cThread() {
    uint8_t* read_buffer = static_cast<uint8_t*>(malloc(max_transfer_));
    if (!read_buffer) {
...
int PlatformI2cBus::I2cThread(void* arg) {
    auto self = static_cast<PlatformI2cBus*>(arg);
    self->I2cThread();
    return 0;
}
